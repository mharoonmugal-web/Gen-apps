...
PRODUCTS = {
    ...
    "Auto Loan": {"rate": KIBOR + 0.05, "max_tenor": 4, "equity": True, "max_limit": 3_000_000, "staff_tenor": 10, "insurance_rate": 0.0175},
...
def staff_loan_schedule(p, r, n):
    """Staff loan: Principle 1 (6/7): Principal only, Markup 2 (1/7): Markup only"""
    monthly_rate = r / 12
...
if submit_button:
    ...
    if staff_loan:
        ...
        with col3:
            ...
            st.metric("Principle 1 EMI", f"PKR {fixed_principal:,.0f}")
            st.metric("Markup 2 EMI", f"PKR {markup_emi:,.0f}")
        ...
        with col1:
            st.markdown(f"**Loan Details:**\n- **Amount:** PKR {approved:,.0f}\n- **Tenor:** {tenor} Years\n- **Principle 1 ({principal_months}m):** PKR {fixed_principal:,.0f}/month\n- **Markup 2 ({markup_months}m):** PKR {markup_emi:,.0f}/month\n- **Total Interest:** PKR {total_markup:,.0f}")
        with col2:
